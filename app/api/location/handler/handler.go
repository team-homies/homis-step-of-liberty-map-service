...
type handler interface {
	...
	FindLocationList(c *fiber.Ctx) error
}
...
// 위치 리스트 조회 query : latitude longitude , header : userId
func (h *locationHandler) FindLocationList(c *fiber.Ctx) error {
	ctx := fiberkit.FiberKit{C: c}

	req := new(resource.FindLocationListRequest)
	ctx.C.BodyParser(req)
	...
	// 3. 서비스 함수 실행
	res, err := h.service.FindLocationList(req.UserId, req)
	if err != nil {
		return ctx.HttpFail(err.Error(), fiber.StatusNotFound)
	}
	return ctx.HttpOK(res)
